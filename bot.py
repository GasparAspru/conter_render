...
PORT = int(os.environ.get("PORT", "8000"))  # Render задаёт PORT автоматически во время выполнения.
...
def main():
    ...
    # Если задан WEBHOOK_URL — запускаем webhook (Render требует слушать порт)
    if WEBHOOK_URL:
        ...
        app.run_webhook(
            listen="0.0.0.0",
            port=PORT,
            webhook_url=full_webhook,  # ✅ обновлено: без webhook_url_path
        )
    else:
